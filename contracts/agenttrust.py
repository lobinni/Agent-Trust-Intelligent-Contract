...
COMPLETED      DISPUTED
    ...
    # Protocol defaults. Owner can change these later.
    ...
    def _load_profile_existing(self, address: Address) -> dict:
        if address not in self.profiles:
    ...
    def set_min_dispute_bond(self, amount: u256):
        self._require_owner()
        if amount == u256(0):
            raise gl.vm.UserError("[EXPECTED] minimum dispute bond must be > 0")
    ...
    # ============================================================
    # 4A. Client approves -> worker settlement
    # ============================================================
    ...
    def approve_task(self, task_id: str):
        ...
        # Settlement is deterministic and happens outside any nondet block.
    ...
    # ============================================================
    # 4B. Permissionless auto-release after review period
    # ============================================================
    ...
    # ============================================================
    # 5. Client opens dispute + posts bond
    # ============================================================
    ...
    def _court_decision(self, task: dict) -> dict:
        """
        Non-deterministic layer only.

        Leader and validator independently fetch the same evidence and
        independently judge it. Consensus is based on stable decision
        fields: winner + score bucket. Free-form reason is not compared.
        """
        ...
        def evaluate():
            ...
            if page.status_code >= 400:
                raise gl.vm.UserError("[EXTERNAL] evidence HTTP error")
            ...
            prompt = f"""
You are the neutral GenLayer Court for an escrow task.
...
"""
            ...
            if len(reason) > int(self.MAX_REASON):
                reason = reason[:int(self.MAX_REASON)]
        ...
        def validator_fn(leader_result):
            ...
            if not isinstance(leader, dict):
                return False
            if leader.get("winner") not in [self.WORKER, self.CLIENT]:
                return False
            ...
            # Settlement-critical field: both nodes must agree on winner.
            if own["winner"] != leader["winner"]:
                return False

            # Score is informational. Allow small model variation.
            if abs(int(own["score"]) - leader_score) > 10:
                return False
    ...
    def adjudicate(self, task_id: str):
        ...
        # IMPORTANT:
        # No storage writes or token transfers happen inside the nondet block.
        # They occur only after consensus returns.
        result = self._court_decision(task)

        if not isinstance(result, dict):
            raise gl.vm.UserError("[COURT_ERROR] invalid consensus result")

        winner = result.get("winner", "")
        if winner not in [self.WORKER, self.CLIENT]:
            raise gl.vm.UserError("[COURT_ERROR] invalid court winner")
        ...
        if winner == self.WORKER:
            # Worker receives escrowed reward + client's dispute bond.
            ...
            # Client receives escrowed reward + bond.
    ...
    def claim_expired(self, task_id: str):
        ...
        # Refund only the original escrowed reward.
    ...
    def get_leaderboard(
        ...
        # Small protocol MVP: deterministic insertion sort by reputation.
        n = len(profiles)
        i = 1
        while i < n:
            current = profiles[i]
